...
class PromptPool(nn.Module):
    def __init__(self, prompters: Iterable[Prompter], embed_dim: int,model):
        super(PromptPool, self).__init__()

        self._num_prompts = len(prompters)

        self.encoder = model.featurizer.encode
        self.forward_head = model.featurizer.inputs_head
    ...
    def parameters(self, recurse: bool=False) -> Iterator[Parameter]:
        yield self.keys
        if recurse:
            yield self.encoder.weight
            if self.encoder.bias is not None:
                yield self.encoder.bias
    ...
    def forward(self, x):  # ...............................| batch_size, C, H, W
        ...
        with torch.no_grad():
            ...
            x_feats,_ = self.encoder(x)
        x_embed = self.forward_head(x_feats)  # ....| batch_size, embed_dim
...
class ProbabilisticPromptPool(nn.Module):
    def __init__(self, prompters: Iterable[Prompter], embed_dim: int, model, 
        ...
        self._num_prompts = len(prompters)
        #model = DAGCN(domain_num)
        self.shared_encoder = model.featurizer.encode
        self.inputs_head =nn.Linear(256, 256)
        
        self.prompt_head = nn.Linear(256, 256)
        self.prompters = nn.ModuleList(prompters)
    ...
    def forward(self, x):  # ...............................| batch_size, C, H, W
        ...
        if self.uniform_mix:
            ...
            with torch.no_grad():
                feats, _ = self.shared_encoder(x_prompts)
